...
- (void)plotGraphData
{
    ...
    float selfheight = self.frame.size.height - 30;
    for (NSUInteger counter = 1; counter <= [_graphData count]; counter++) {
        ...
        float screenHeight = (selfheight/2 - (offsets)) / (selfheight/2 + offSetFromTop + offsetFromBottom);
        
        CGPoint point = CGPointMake(xCoord,
                                    selfheight/2 - (([[_graphData objectAtIndex:counter - 1] integerValue] *
                                                               ((selfheight/2 * screenHeight) / range)) -
                                                              (lowest * ((selfheight/2 * screenHeight) / range ))+
                                                              offsetFromBottom));
        ...
        if (self.useCurvedLine == NO) {
            ...
            if (lastPoint.x != 0) {
                if (!self.hideLines) {
                    ...
                    [self drawLineBetweenPoint:lastPoint andPoint:point withColour:counter == 2 ? [_strokeColor colorWithAlphaComponent:.5]:_strokeColor];
                }
            }
...
//    画下面的图形
    ...
    [pointsCenterLocations removeAllObjects];
    
    graphRange = [self workOutRangeFromArray:_belowGraphData];
    range = [[graphRange objectForKey:@"range"] integerValue];
    lowest = [[graphRange objectForKey:@"lowest"] integerValue];
    highest = [[graphRange objectForKey:@"highest"] integerValue];
    ...
    CGPoint slastPoint = CGPointMake(0, 0);
    
    for (NSUInteger counter = 1; counter <= [_belowGraphData count]; counter++) {
        NSInteger xCoord = (self.graphWidth / [_belowGraphData count]) * counter;
        ...
        CGPoint point = CGPointMake(xCoord,
                                    selfheight - (([[_belowGraphData objectAtIndex:counter - 1] integerValue] *
                                                                 ((selfheight/2 * screenHeight) / range)) -
                                                                (lowest * ((selfheight/2 * screenHeight) / range ))+
                                                                offsetFromBottom));
        if (self.hideLabels == NO) {
            //            todo温度
            [self createBelowPointLabelForPoint:point withLabelText:[NSString stringWithFormat:@"%@℃",[_belowGraphData objectAtIndex:counter - 1]]];
        }
        ...
        if (self.useCurvedLine == NO) {
            ...
            if (slastPoint.x != 0) {
                if (!self.hideLines) {
                    ...
                    [self drawLineBetweenPoint:slastPoint andPoint:point withColour:counter == 2 ? [_strokeColor colorWithAlphaComponent:.5]:_strokeColor];
                }
            }
...
- (void)createBelowPointLabelForPoint:(CGPoint)point withLabelText:(NSString *)string{
    UILabel *tempLabel = [[UILabel alloc] initWithFrame:CGRectMake(point.x , point.y, 30, kPointLabelHeight)];
    tempLabel.textAlignment = NSTextAlignmentCenter;
    [tempLabel setTextColor:self.labelFontColor];
//    [tempLabel setBackgroundColor:self.labelBackgroundColor];
    ...
    [tempLabel setAdjustsFontSizeToFitWidth:YES];
    [tempLabel setMinimumScaleFactor:0.6];
    [tempLabel setCenter:CGPointMake(point.x, point.y+kPointLabelHeight)];
    [_graphView addSubview:tempLabel];
    [tempLabel setText:string];
}
                         ...
                         fromArray:(NSMutableArray *)pointsArray
{
    ...
    for (int i = 0; i < [pointCenterLocations count]; i++) {
        CGRect pointRect = CGRectMake(0, 0, 10, 10);
//        todo 划圆圈
        JYGraphPoint *point = [[JYGraphPoint alloc] initWithFrame:pointRect];
        
        [point setStrokeColour:stroke];
        [point setFillColour:i == 0 ?[fill colorWithAlphaComponent:.5]:fill];
